...
func NewDiskLayerStore(rootPath string, db *DB) *DiskLayerStore {
...
func (s *DiskLayerStore) getLayerPath(digest string) string {
	return filepath.Join(s.RootPath, "sha256", digest)
}

func (s *DiskLayerStore) readFilesFromArchive(digest string) ([]layers.FileRecord, error) {
	digestPath := s.getLayerPath(digest)
	log.Infof(context.Background(), "Reading files from layer: %s @ %s", digest, digestPath)
	files := make([]layers.FileRecord, 0)
	if file, err := os.Open(digestPath); err == nil {
		...
		for {
			...
			files = append(files, layers.FileRecord{Name: hdr.Name, Size: hdr.Size, IsDir: false})
		}
	}
...
func (s *DiskLayerStore) LayerFileNames(digest string) ([]string, error) {
...
func (s *DiskLayerStore) IngestFilesForLayer(digest string) error {
...
func (s *DiskLayerStore) ReadFile(layerDigest string, filename string) ([]byte, error) {
...
func (s *DiskLayerStore) GetDirectoryTree(layerDigest string) ([]layers.FileRecord, error) {
	return s.readFilesFromArchive(layerDigest)
}

func (s *DiskLayerStore) DeleteLayer(hex string) error {
