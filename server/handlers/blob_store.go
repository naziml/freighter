...
func (s *IndexingBlobStore) Stat(_ context.Context, _ string, h v1.Hash) (int64, error) {
	layer, err := s.store.GetLayer(types.Digest{Algorithm: h.Algorithm, Hash: h.Hex})
	if err != nil {
...
func (s *IndexingBlobStore) Get(_ context.Context, _ string, h v1.Hash) (io.ReadCloser, error) {
	rc, err := s.store.GetLayerReader(types.Digest{Algorithm: h.Algorithm, Hash: h.Hex})
	if err != nil {
...
func (s *IndexingBlobStore) Put(ctx context.Context, repo string, h v1.Hash, rc io.ReadCloser) error {
	...
	if err := s.store.StoreLayerBlob(types.Digest{Algorithm: h.Algorithm, Hash: h.Hex}, rc); err != nil {
	...
	if _, err := gzip.NewReader(rc); err != nil {
		...
		err := s.store.IngestLayer(ctx, types.Digest{Algorithm: h.Algorithm, Hash: h.Hex})
		if err != nil {
...
func (s *IndexingBlobStore) Delete(ctx context.Context, _ string, h v1.Hash) error {
	...
	if err := s.store.DeleteLayer(types.Digest{Algorithm: h.Algorithm, Hash: h.Hex}); err != nil {
